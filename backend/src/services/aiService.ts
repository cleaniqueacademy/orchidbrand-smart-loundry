...
/**
 * Mengumpulkan data operasional outlet secara realtime untuk konteks Gemini
 */
async function getOperationalContext(tenantId: string | null) {
  if (!tenantId) {
    return {
...
/**
 * Fallback cerdas jika GEMINI_API_KEY belum disetel di .env atau offline
 * Menguasai 100% informasi Menu, Settings, Alur Kasir, Shift, dan Operasional Laundry.
 */
function generateLocalFallbackReply(message: string, context: any, role: string): string {
  const lower = message.toLowerCase();

  // 1. PANDUAN PENGATURAN & SETTINGS
  if (
  ...
  // 2. PANDUAN WHATSAPP GATEWAY & SCAN QR
  ...
  // 3. PANDUAN SHIFT KASIR & UANG LACI
  ...
  // 4. PANDUAN ALUR KASIR POS & STATUS ORDER
  if (
  ...
  // 5. PANDUAN BUKU KAS, LAPORAN & KEUANGAN
  if (
  ...
  // 8. PANDUAN LANGGANAN & INVOICE OUTLET
  ...
  // 9. KATALOG SELURUH MENU (Overview Navigation)
  if (
...
/**
 * Panggil API Google Generative AI (Gemini Flash)
 */
async function callGeminiAPI(
  ...
  if (!res.ok) {
    ...
    throw new Error(`Gemini API error [HTTP ${res.status}]: ${errorBody}`);
  }

  const json: any = await res.json();
  const text =
    json?.candidates?.[0]?.content?.parts?.[0]?.text ||
    "Maaf, tidak ada respon dari model AI.";
  return text;
}
...
export async function askLaundryAssistant(options: ChatOptions): Promise<AIResponse> {
  ...
  // Jika API Key belum dikonfigurasi, gunakan mesin lokal cerdas
  if (!apiKey) {
...
- Role-aware: jika user adalah kasir (staff), jangan sarankan menu yang hanya bisa diakses owner (seperti buku kas pengeluaran atau hapus order).
`.trim();
  ...
  try {
    ...
    try {
      reply = await callGeminiAPI(apiKey, "gemini-2.5-flash", systemPrompt, message, history);
    } catch (modelErr: any) {
      console.warn("[Gemini] gemini-2.5-flash fallback to gemini-1.5-flash:", modelErr.message);
      activeModel = "gemini-1.5-flash";
      reply = await callGeminiAPI(apiKey, "gemini-1.5-flash", systemPrompt, message, history);
    }
    ...
    const localReply = generateLocalFallbackReply(message, context, userRole);
    return {
      reply: `${localReply}\n\n*(Catatan: Mode AI offline digunakan karena koneksi Gemini API sedang mengalami kendala: ${err.message})*`,
