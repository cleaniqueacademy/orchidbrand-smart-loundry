...
export const CashflowTab: React.FC<CashflowTabProps> = ({
  ...
  const unifiedTransactions = useMemo<CashflowItem[]>(() => {
    ...
    const manualItems: CashflowItem[] = expenses.map((exp) => ({
      ...
      notes: exp.notes,
      amount: exp.amount,
      rentDurationMonths: exp.rentDurationMonths,
    ...
    },
    {
      ...
      cell: (item) => (
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {item.rentDurationMonths && item.rentDurationMonths > 1 && (
            <span className="inline-flex items-center text-[9px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 px-1.5 py-0.5 rounded whitespace-nowrap">
              Sewa {item.rentDurationMonths} Bln
            </span>
          )}
          {item.isAutoGenerated === "true" && (
            <span className="inline-flex items-center text-[9px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 px-1.5 py-0.5 rounded whitespace-nowrap">
              Auto App
            </span>
          )}
        </div>
